<x-layout>
    <div style="margin-left: 30px; margin-top: 20px">
        <h1 class="mb-4 text-3xl font-extrabold text-gray-900 dark:text-white md:text-5xl lg:text-4xl"><span
                class="text-transparent bg-clip-text bg-gradient-to-r to-blue-800 from-blue-300">Routes</span></h1>
    </div>

    <div>
        <div style="display: flex; flex-direction: column; align-items: center">

            <a href="{{route('routes.create')}}" style="width: 50%">
                <button
            ...
            <a href="{{ route('locations.index') }}" style="width: 50%">
                <button
                    class="mt-5 tracking-wide font-semibold bg-blue-400 text-black w-full rounded-lg hover:bg-blue-400 transition-all duration-300 ease-in-out flex items-center justify-center focus:shadow-outline focus:outline-none">
                    <span>
                        Location Management
        ...
        <div
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3" style="text-align: center;">Source ID</th>
                    <th scope="col" class="px-6 py-3" style="text-align: center;">Source City</th>
                    <th scope="col" class="px-6 py-3" style="text-align: center;">Destination ID</th>
                    <th scope="col" class="px-6 py-3" style="text-align: center;">Destination City</th>
                    <th scope="col" class="px-6 py-3" style="text-align: center;">Vehicle ID</th>
                    <th scope="col" class="px-6 py-3" style="text-align: center;">Price</th>
                ...
                @foreach($routes as $route)
                    <tr class="bg-gray-100">
                        ...
                        <td style="text-align: center;">{{$route->source}}</td>
                        <td style="text-align: center;">{{$route->source_location->city}}</td>
                        <td style="text-align: center;">{{$route->destination}}</td>
                        <td style="text-align: center;">{{$route->destination_location->city}}</td>
                        <td style="text-align: center;">{{$route->vehicle_id}}</td>
                        <td style="text-align: center;">{{$route->price}}</td>
                        <td style="text-align: center;">{{$route->departure_time}}</td>
                        <td style="text-align: center;">{{$route->arrival_time}}</td>
                        <td style="display: flex; align-items: center; justify-content: center; gap: 10px;">
                            <form
                                action="{{route('routes.edit', $route)}}">
                                @csrf
                                @method('GET')
                            ...
                            <form
                                method="POST" action="{{route('routes.destroy', $route)}}">
                                @csrf
                                @method('DELETE')
